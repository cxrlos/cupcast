"""Group-stage scorer and FIFA tiebreaker ranker for the v2 Monte Carlo sim."""
...
def sample_group_scores(
    ...
    for m, (i, j) in enumerate(PAIRS):
        matrix = predict.score_matrix(
            posterior, teams[i], teams[j],
            host_home=hosts[i], host_away=hosts[j], max_goals=MAX_GOALS,
        )
