"""Monte Carlo tournament driver: 50k simulations of the full 2026 World Cup."""
...
from cupcast.v2.sim.bracket import allocate_thirds
from cupcast.v2.sim.group_stage import composite_key, rank_group, sample_group_scores
...
def run_tournament(
    posterior, n_sims: int = 50_000, seed: int = 2026
) -> pd.DataFrame:
    return simulate_tournament(posterior, n_sims, seed).table
...
def simulate_tournament(
    ...
    for g, letter in enumerate(GROUP_LETTERS):
        ...
        home_goals, away_goals = sample_group_scores(posterior, teams, hosts, n_sims, rng)
    ...
    sampler = KnockoutSampler(posterior, ALL_TEAMS, gk_z)
    match_winner: dict[int, np.ndarray] = {}
    match_loser: dict[int, np.ndarray] = {}
    ...
    # p_r16 counts each team once per sim (16 teams advance; bincount is /n_sims not /2*n_sims).
    # The R32 produces 16 winners out of 32 matches, so concatenating all 16 winners arrays and
    # dividing by n_sims gives the correct per-team fraction.
    table = pd.DataFrame(
        {
            "team": ALL_TEAMS,
