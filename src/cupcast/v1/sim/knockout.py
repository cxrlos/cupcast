...
from cupcast.v1.sim.worldcup2026 import HOST_COUNTRIES
...
def advance_probability(
    ...
    regulation = fit.score_matrix(team, opponent, host_home=host_team, host_away=host_opp)
    p_win = float(np.tril(regulation, -1).sum())
    p_draw = float(np.trace(regulation))
    extra = fit.score_matrix(
        team, opponent, host_home=host_team, host_away=host_opp, rate_scale=EXTRA_TIME_SCALE
    )
